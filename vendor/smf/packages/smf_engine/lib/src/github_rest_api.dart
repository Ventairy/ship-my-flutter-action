...
final class GitHubRestApi implements GitHubApi {
  ...
  Future<http.Response> _send(
    ...
    try {
      ...
      throw SmfError('Could not reach GitHub.', SmfErrorCode.githubApi, cause: error);
    }
  ...
  Future<List<GitHubPullRequestDto>> listPullRequests({
    ...
    return <GitHubPullRequestDto>[for (final item in data) _pullRequest(item)];
  }
  ...
  @override
  Future<GitHubReleaseDto?> releaseByTag(String tag) async {
    try {
      final response = await _send(
        'GET',
  ...
  GitHubPullRequestDto _pullRequest(Object? value) {
    return _decodeDto(value, GitHubPullRequestDto.fromJson);
  }

  GitHubReleaseDto _release(Object? value) {
    return _decodeDto(value, GitHubReleaseDto.fromJson);
  }
