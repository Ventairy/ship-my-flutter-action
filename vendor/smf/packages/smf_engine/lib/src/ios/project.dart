  ...
  static Future<({String option, String path})> _xcodeContainer(
    ...
    if (!await directory.exists()) {
      throw SmfError(
        ...
        SmfErrorCode.xcodeProjectNotFound,
      );
    }
    final entries =
    ...
    Directory? select(List<Directory> matchingDirectories, String preferredName) {
      final preferred = matchingDirectories.where((item) => p.basename(item.path) == preferredName).firstOrNull;
      if (preferred != null) return preferred;
      if (matchingDirectories.length == 1) return matchingDirectories.single;
      if (matchingDirectories.length > 1) {
        throw SmfError(
          'Multiple Xcode containers were found in $iosDirectory: '
          '${matchingDirectories.map((item) => p.basename(item.path)).join(', ')}.',
