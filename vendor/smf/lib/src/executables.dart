...
String _usage(String name, String description, ArgParser parser) =>
    '''
Usage: dart run smf:$name [options]
...
Future<int> _runExecutable({
  ...
  try {
    ...
    resolvedIo.writeError('smf:$name: ${error.message}');
    resolvedIo.writeError(_usage(name, description, parser));
    return 64;
  } on SmfError catch (error) {
    resolvedIo.writeError('smf:$name [${error.code}]: ${error.message}');
    return 1;
  } on GitHubApiException catch (error) {
    resolvedIo.writeError(
      'smf:$name [GITHUB_API]: GitHub ${error.method} ${error.path} '
      'failed (${error.statusCode}).',
    );
    return 1;
  } on dart_io.FileSystemException catch (error) {
    resolvedIo.writeError('smf:$name [FILESYSTEM]: ${error.message}');
    return 1;
  }
}
